"""
    Problem Set 5: Recognition Using Deep Networks: Task 3
    ...
    This file will read in the trained NeuralNetwork. It will then read in and transform an example set of 
    provided Greek letters. It will train the NeuralNetwork on these letters and plot the results.
...
def main(argv):
    ...
    greek_letters_folder = 'greek_train'
    homebrew_greek_letters_folder = 'greek_train_homebrew'
    ...
    # change the last layer of the model and display
    ...
    greek_letters_folder = os.path.join(path, greek_letters_folder)
    ...
    figure_name = "greek_letters_losses_and_accuracy.png"
    title_suffix = "Greek Letters"
    ...
    # set the path to homebrew greek letters folder
    greek_letters_folder = os.path.join(path, homebrew_greek_letters_folder)
    if not os.path.exists(greek_letters_folder):
        raise FileNotFoundError("Greek Letters Folder Not Found!")

    # get the greek letters
    greek_train_homebrew = get_transform_greek_letters(greek_letters_folder, batch_size, True)
    ...
    # plot some examples
    greek_letter_example_fig = 'homebrew_greek_example_images.png'
    plot_example_images_with_labels(example_data, example_target, 5, path, greek_letter_example_fig)

    # train model for 12 epochs
    n_epochs = 15
    for i in range(n_epochs):
        train_losses_list, train_accuracy_list, train_counter_list = train_loop(greek_train_homebrew, model, optimizer, train_losses_list, train_accuracy_list, 
                                                                            train_counter_list, i, log_interval, batch_size)
    # plot results
    epochs_for_x_axis = np.arange(1, 16, 1)
    figure_name = "homebrew_greek_letters_losses_and_accuracy.png"
    title_suffix = "Greek Letters Homebrew"
    plot_greek_accuracy_and_loss(epochs_for_x_axis, train_accuracy_list, train_losses_list, title_suffix, path, figure_name)
